...
If the game context and narrative brief are both empty, placeholder text, or too thin to build on, say so explicitly and ask for the missing specifics rather than inventing a setting or story.
...
- **Added**: Explicit missing-data fallback — if the game context and narrative brief are both empty or too thin, the prompt now says so and asks for specifics instead of inventing a setting or story.
